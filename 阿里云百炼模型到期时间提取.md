...
本脚本由 **阿里百炼平台qwen-coder** 完成，专为阿里云百炼平台使用，可一键自动提取模型市场中的模型信息，包括：
...
> ⚠️ 请确保已登录阿里云并进入【[模型广场](https://bailian.console.aliyun.com/console?tab=model#/model-market)】页面再点击提取按钮。
>
> ⚠️ ~~【[模型广场](https://bailian.console.aliyun.com/console?tab=model#/model-market)】主页面提取需要先手动「点击切换列表显示![](.assert/阿里云百炼模型到期时间提取/05.others.png)」->「点击展开」~~(**最新版本现在可以一键自动切换列表视图并提取数据了！！！**)
>
> ⚠️ 单个模型的「详情页面」则直接点击『提取模型信息』即可，比如[通义千问3-Coder-Plus](https://bailian.console.aliyun.com/console?tab=model#/model-market/detail/group-qwen3-coder-plus?modelGroup=group-qwen3-coder-plus)、[通义千问3](https://bailian.console.aliyun.com/console?tab=model#/model-market/detail/qwen3?modelGroup=qwen3)
...
- 更快速的初始化：
  ...
  - setTimeout(createFloatingButton, 50) 替代原来的 setTimeout(createFloatingButton, 500)

- 减少冗余等待：

  - 每个步骤等待时间从300-800ms缩短到50-150ms
