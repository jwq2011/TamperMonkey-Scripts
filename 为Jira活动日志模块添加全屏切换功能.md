...
本脚本在 Jira 问题页面的「活动日志（Activity Log）」模块标题右侧**动态添加多个功能按钮**：

- **默认不自动开启全屏** — 页面加载后保持原布局，不干扰用户。
- **点击「全屏模式」** → 活动日志模块全屏展开，隐藏侧边栏和其他模块，专注查看日志。
- **点击「退出全屏」** → 恢复原始布局，所有隐藏元素重新显示。
- **点击「AI 分析」** → 提取活动日志内容并提交至阿里百炼大模型进行分析，结果以弹窗形式展示。
- **点击「模板」** → 打开模板编辑器，可编辑、保存和复制预设模板内容到剪贴板。
- **点击「配置」** → 打开配置面板，可设置 API Key、模型名称和分析模板。
...
2. **动态插入按钮**
   在标题栏末尾创建多个功能按钮，样式采用现代 UI 设计（圆角、阴影、悬停过渡），与 Jira 原生风格协调。
...
1. 在 `#activitymodule_heading` 内创建多个按钮：
   - "全屏模式"按钮：点击后将活动模块设为全屏（`position: fixed; top: 70px; width: 100%; height: calc(100% - 70px);`），隐藏侧边栏 `.aui-sidebar` 和 `.issue-body-content` 内除活动模块和 `#stalker`（固定顶栏）外的所有 `.module`，按钮文字变为"退出全屏"，颜色变红
   - "AI 分析"按钮：点击后提取 `.mod-content` 内容并提交至阿里百炼大模型分析
   - "模板"按钮：点击后打开模板编辑器，可编辑、保存和复制预设模板
   - "配置"按钮：点击后打开配置面板，可设置 API Key、模型名称和分析模板
2. 再次点击"退出全屏"按钮恢复原状，显示所有隐藏元素，按钮恢复蓝色
...
## 📝 模板功能说明
点击"模板"按钮，将打开模板编辑器，具有以下功能：
- ✅ 预设默认模板，可直接复制到剪贴板
- ✅ 可编辑模板内容并保存，下次使用时会加载上次保存的版本
- ✅ 支持重置为默认模板
- ✅ "复制到剪贴板"按钮可将当前模板内容复制到系统剪贴板
...
- **分析模板**：支持选择预设分析模板（项目经理分析、JIRA专家分析）
